...
3. Intelligently decides whether to:
...
Run with: `uvicorn twilio:app --host 0.0.0.0 --port 8000`
"""
...
# Set browser to non-interactive mode BEFORE importing browser_tasks
    ...
    try:
        response = await openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """You are a query interpreter for a voice-based AI assistant. The user speaks commands that get transcribed, sometimes with errors or unclear phrasing.

Your job: Convert the raw transcript into a clear, actionable query.

Guidelines:
- Fix obvious transcription errors (e.g., "leachess.org" → "lichess.org")
- Remove filler words ("um", "uh", "like")
- Clarify ambiguous requests (e.g., "make account" → "create an account")
...
Output: "Create an account on lichess.org"

Input: "search for uh browser automation stuff"
Output: "Search for browser automation"
...
Respond with ONLY the clarified query - nothing else."""
        ...
        print(f"[ERROR] Query clarification failed: {e}, using original")
        return raw_query  # Fallback to original on error


async def decide_use_browser(query: str) -> bool:
    """Decide if the query needs browser automation or can use Perplexity.
...
- Weather, time, simple lookups

Respond with ONLY 'BROWSER' or 'PERPLEXITY' - nothing else."""
                },
                {
                    "role": "user",
                    "content": query
        ...
        use_browser = "BROWSER" in decision
        print(f"[DECISION] Query: '{query}' -> {'BROWSER' if use_browser else 'PERPLEXITY'}")
        return use_browser
    except Exception as e:
        print(f"[ERROR] Decision failed: {e}, defaulting to Perplexity")
        return False  # Default to Perplexity on error
        ...
        async def process_and_respond():
            ...
            try:
                ...
                if use_browser:
                    ...
                    try:
                        # Run browser task with clarified query
                        ...
                        if browser_result.get("successful_count", 0) > 0:
                            # Get the actual result from the first successful task
                            results = browser_result.get("execution_results", [])
                            browser_response = "I've completed the browser task. "
                            if results and results[0].success:
                                result_data = results[0].result
                                browser_response += f"Here's what I found: {str(result_data)}"
                            else:
                                browser_response += "The task was completed successfully."
                        else:
                            browser_response = "I tried to complete the task using a browser, but encountered some difficulties. Let me provide general information instead."
                            # Fallback to Perplexity
                            use_browser = False
                        
                        if use_browser:
                            simplified_response = await simplify_response(browser_response)
                    except Exception as e:
                        print(f"[ERROR] Browser automation failed: {e}")
                        simplified_response = "I had trouble accessing the browser. Let me try another way to help you."
                        use_browser = False
