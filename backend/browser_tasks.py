...
class ParallelBrowserSDK:
    ...
    def _create_llm(self) -> ChatGoogle:
        ...
        return ChatGoogle(model=self.llm_model, api_key=self.llm_api_key)
    ...
    async def execute_parallel(
        ...
        try:
            ...
            for task in tasks:
                ...
                agent = Agent(task=task.task_description, browser=browser, llm=llm)
                agents.append((agent, task))

            agent_tasks = [agent.run(max_steps=task.max_steps) for agent, task in agents]
...
async def run_parallel_tasks(
    ...
    tasks = [ParallelTask(task_description=desc, max_steps=max_steps) for desc in task_descriptions]
